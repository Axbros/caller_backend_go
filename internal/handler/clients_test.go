...
func newClientsHandler() *gotest.Handler {
	...
	d.IDao = dao.NewClientsDao(d.DB, c.ICache.(cache.ClientsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &clientsHandler{iDao: d.IDao.(dao.ClientsDao)}
	iHandler := h.IHandler.(ClientsHandler)
...
func Test_clientsHandler_Create(t *testing.T) {
	h := newClientsHandler()
	defer h.Close()
	testData := &types.CreateClientsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Clients))
...
func Test_clientsHandler_DeleteByID(t *testing.T) {
	h := newClientsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Clients)
...
func Test_clientsHandler_UpdateByID(t *testing.T) {
	h := newClientsHandler()
	defer h.Close()
	testData := &types.UpdateClientsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Clients))
...
func Test_clientsHandler_GetByID(t *testing.T) {
	h := newClientsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Clients)
...
func Test_clientsHandler_List(t *testing.T) {
	h := newClientsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Clients)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("List"), &types.ListClientssRequest{query.Params{
		Page: 0,
		Size: 10,
...
func Test_clientsHandler_DeleteByIDs(t *testing.T) {
	h := newClientsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Clients)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteClientssByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = gohttp.Post(result, h.GetRequestURL("DeleteByIDs"), &types.DeleteClientssByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_clientsHandler_GetByCondition(t *testing.T) {
	h := newClientsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Clients)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("GetByCondition"), &types.GetClientsByConditionRequest{
		query.Conditions{
...
func Test_clientsHandler_ListByIDs(t *testing.T) {
	h := newClientsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Clients)
	...
	result := &gohttp.StdResult{}
	err := gohttp.Post(result, h.GetRequestURL("ListByIDs"), &types.ListClientssByIDsRequest{IDs: []uint64{testData.ID}})
	if err != nil {
		t.Fatal(err)
	}
	if result.Code != 0 {
	...
	// get error test
	err = gohttp.Post(result, h.GetRequestURL("ListByIDs"), &types.ListClientssByIDsRequest{IDs: []uint64{111}})
	assert.Error(t, err)
}

func Test_clientsHandler_ListByLastID(t *testing.T) {
	h := newClientsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Clients)
...
func TestNewClientsHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewClientsHandler()
