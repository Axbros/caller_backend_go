...
graph:
	...
	@cp -f cmd/caller/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=caller caller
	@rm -f main.go caller.gv
...
build:
	...
	@cd cmd/caller && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
