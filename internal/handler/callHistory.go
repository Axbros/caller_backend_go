...
type CallHistoryHandler interface {
	...
	GetByUserMachineCode(c *gin.Context)
...
// GetByUserMachineCode get a record by user machine code
// @Summary get callHistory detail
// @Description get callHistory detail by machine code
// @Tags callHistory
// @Param id path string true "user_machine_code"
// @Accept json
// @Produce json
// @Success 200 {object} types.GetCallHistoryByIDRespond{}
// @Router /api/v1/callHistory/{user_machine_code} [get]
// @Security BearerAuth
func (h *callHistoryHandler) GetByUserMachineCode(c *gin.Context) {
...
func (h *callHistoryHandler) GetByCondition(c *gin.Context) {
	...
	ctx := middleware.WrapCtx(c)
	callHistories, err := h.iDao.GetByCondition(ctx, &form.Conditions)
	if err != nil {
		if errors.Is(err, model.ErrRecordNotFound) {
	...
	// data := &types.CallHistoryObjDetail{}
	// err = copier.Copy(data, callHistories)
	// if err != nil {
	// 	response.Error(c, ecode.ErrGetByIDCallHistory)
	// 	return
	// }
	data := make([]*types.CallHistoryObjDetail, len(callHistories))

	for i, callHistory := range callHistories {
		data[i] = &types.CallHistoryObjDetail{} // 初始化每个元素
		err = copier.Copy(data[i], callHistory)
		if err != nil {
			response.Error(c, ecode.ErrGetByIDCallHistory)
			return
		}
		data[i].ID = utils.Uint64ToStr(callHistory.ID)
	}
	// Note: if copier.Copy cannot assign a value to a field, add it here
...
func getCallHistoryUserMachineCodeFromPath(c *gin.Context) (string, uint64, bool) {
	idStr := c.Param("user_machine_code")
	id, err := utils.StrToUint64E(idStr)
	if err != nil || id == 0 {
		logger.Warn("StrToUint64E error: ", logger.String("idStr", idStr), middleware.GCtxRequestIDField(c))
		return "", 0, true
